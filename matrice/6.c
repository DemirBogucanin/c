#include <stdio.h>

void unosMatrice(int A[100][100], int redovi, int kolone) {
    printf("Unesite matricu A:\n");
    for (int i = 0; i < redovi; i++) {
        for (int j = 0; j < kolone; j++) {
            scanf("%d", &A[i][j]);
        }
    }
}

int indeksNajvecegElementa(int A[100][100], int i, int kolone) {
    int maxIndex = 0;
    for (int j = 1; j < kolone; j++) {
        if (A[i][j] > A[i][maxIndex]) {
            maxIndex = j;
        }
    }
    return maxIndex;
}

void formirajNizB(int A[100][100], int B[100], int redovi, int kolone) {
    for (int i = 0; i < redovi; i++) {
        B[i] = indeksNajvecegElementa(A, i, kolone);  // Formiramo niz B
    }
}

void ispisiNiz(int B[100], int redovi) {
    printf("Niz B:\n");
    for (int i = 0; i < redovi; i++) {
        printf("%d ", B[i]);
    }
    printf("\n");
}

int main() {
    int A[100][100], B[100];
    int redovi, kolone;
    

    printf("Unesite broj redova i kolona: ");
    scanf("%d %d", &redovi, &kolone);

    unosMatrice(A, redovi, kolone);
    
   
    formirajNizB(A, B, redovi, kolone);
    

    ispisiNiz(B, redovi);
    
    return 0;
}
